...
function makePlan(k=day()){
 const l=latinStats(),f=frenchStats(),tasks=[];
 ...
 if(tasks.length<3&&l.weak>0&&!tasks.some(t=>t.subject==='latin'))tasks.push({id:`latin-weak-${k}`,subject:'latin',reason:'Weak area',title:'Repair weak Latin',target:7,minutes:2,kind:'latin-weak',baseline:l.answers});
 if(tasks.length<3&&f.loaded&&f.weak>0&&!tasks.some(t=>t.subject==='french'))tasks.push({id:`french-weak-${k}`,subject:'french',reason:'Weak area',title:'Repair weak French',target:7,minutes:2,kind:'french-weak',baseline:f.answers});
 if(tasks.length<3){
...
function taskProgress(t,k=day()){
 const l=latinStats(),f=frenchStats();
